...
import { Inbox, CheckCircle, XCircle, Clock, Baby, AlertCircle, MapPin } from 'lucide-react';
...
const IncomingRequests = () => {
    ...
    const fetchRequests = async () => {
        ...
        try {
            ...
            // Filter: Only show Incoming requests (where isOutgoing === false)
            const incoming = data.filter(r => !r.isOutgoing);
    ...
    const handleAction = async () => {
        ...
        try {
            ...
            if (status === 'Completed') showFeedback('success', "Stock dispatched successfully!");
                        ...
                        <div className="flex gap-3">
                            ...
                            <button
                                ...
                                className={`flex-1 py-3 text-white font-bold rounded-xl shadow-lg transition-transform active:scale-95
                                    ${confirmAction.status === 'Rejected' ? 'bg-error shadow-red-200' : 'bg-primary shadow-primary/20'}`}
                            >
                                Confirm
                            </button>
                        </div>
