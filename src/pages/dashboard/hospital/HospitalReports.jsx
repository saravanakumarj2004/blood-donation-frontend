            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
                        <BarChart className="text-blue-600" size={32} /> Blood Reports
                    </h1>
                ...
                <div className="flex bg-slate-100 p-1 rounded-lg border border-slate-200">
                    {['Week', 'Month', 'Year'].map(t => (
                        <button
                            ...
                            className={`px-5 py-1.5 rounded-md text-sm font-semibold transition-all ${filter === t.toLowerCase() ? 'bg-white shadow-sm text-blue-700' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-200/50'}`}
                        >
                            {t}
                        </button>
                    ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden group">
                            ...
                            <div className="mt-4 flex gap-2">
                                <span className="text-xs font-semibold bg-blue-50 text-blue-700 px-2 py-1 rounded-md">Whole: {stats.usage.whole}</span>
                        ...
                        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 relative overflow-hidden group bg-gradient-to-br from-white to-emerald-50/30">
                            ...
                            <div className="flex gap-8">
                                ...
                                <div>
                                    <div className="text-2xl font-bold text-blue-600">-{stats.transfers.outgoing}</div>
                            ...
                            {stats.history.map(item => (
                                <div key={item.id} className="flex items-center justify-between p-4 bg-slate-50 rounded-xl hover:bg-slate-100 border border-slate-100 transition-colors">
                                    <div className="flex items-center gap-4">
                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center ${item.type === 'Usage' ? 'bg-blue-100 text-blue-600' :
                                    ...
                                    <div className="text-xs font-semibold text-slate-600 bg-white px-2.5 py-1 rounded-md border border-slate-200">
                                        {item.date}
                                    </div>
                                </div>
                            ))}
